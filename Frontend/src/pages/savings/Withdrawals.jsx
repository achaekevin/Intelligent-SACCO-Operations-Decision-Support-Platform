...
const Withdrawals = () => {
  ...
  const fetchRecentTransactions = async () => {
    try {
      const token = localStorage.getItem('accessToken')
      const response = await axios.get(`${API_URL}/savings/transactions?type=withdrawal&limit=20`, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const isWithdrawalValid = () => {
    if (!selectedAccount || !withdrawAmount) return true
    const newBalance = parseFloat(selectedAccount.balance) - parseFloat(withdrawAmount)
    return newBalance >= parseFloat(selectedAccount.minimumBalance || 0)
  }
  ...
  return (
    <div>
      <PageHeader
        title="Withdrawals"
        subtitle="Process member withdrawals from savings accounts"
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {selectedAccount && (
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                ...
                <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                  Minimum Balance: {formatKES(selectedAccount.minimumBalance || 0)}
                </p>
            ...
            {selectedAccount && withdrawAmount && !isWithdrawalValid() && (
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-600 dark:text-red-400 mt-1">
                    This withdrawal would violate the minimum balance requirement of {formatKES(selectedAccount.minimumBalance || 0)}
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="space-y-4">
          <Card>
            <h3 className="text-sm font-semibold text-ink-700 dark:text-ink-200 mb-3">Today's Withdrawals</h3>
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-ink-400">Number of Withdrawals</span>
      ...
      {/* Recent Transactions */}
      <Card>
        <h3 className="text-lg font-semibold text-ink-800 dark:text-ink-50 mb-4">Recent Withdrawals</h3>
        {loadingTransactions ? (
